...
float posX =0, posY = 2.5, posZ =-3.5, rotRodDer = 0, rotRodIzq = 0, giroBrazoIzq = 0, giroBrazoDer;
float giroMonito = 0;

#define MAX_FRAMES 80000000000
int i_max_steps = 90;
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float giroMonito;
	float giroMonitoInc;
	float giroBrazoIzq;//variable
	float giroBrazoIzqInc;//incremento
	float giroBrazoDer;//variable
	float giroBrazoDerInc;//incremento
...
void saveFrame ( void )
{
	...
	KeyFrame[FrameIndex].giroBrazoIzq = giroBrazoIzq;//guardando frame paso 3
	KeyFrame[FrameIndex].giroBrazoDer = giroBrazoDer;//guardando frame paso 3	
	FrameIndex++;
}
...
void resetElements( void )
{
	...
	giroBrazoIzq = KeyFrame[0].giroBrazoIzq; // resetear frame paso 4
	giroBrazoDer = KeyFrame[0].giroBrazoDer; // resetear frame paso 4
}
...
void interpolation ( void )
{
	...
	KeyFrame[playIndex].giroBrazoIzqInc = (KeyFrame[playIndex + 1].giroBrazoIzq - KeyFrame[playIndex].giroBrazoIzq) / i_max_steps; // paso 4
	KeyFrame[playIndex].giroBrazoDerInc = (KeyFrame[playIndex + 1].giroBrazoDer - KeyFrame[playIndex].giroBrazoDer) / i_max_steps; // paso 4
	KeyFrame[playIndex].rotIncDer = (KeyFrame[playIndex + 1].rotRodDer - KeyFrame[playIndex].rotRodDer) / i_max_steps; // paso 4
}
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				glRotatef(-45 + giroBrazoDer, 0, 0, 1);
				glRotatef(-45 + giroBrazoDer, 1, 0, 0);
		...
		glPushMatrix(); //Brazo izquierdo <--
			...
			glPushMatrix();
				...
				glRotatef(25+giroBrazoIzq, 1, 0, 0); 
				glTranslatef(-0.75, 0, 0);
				fig7.prisma(0.7, 1.5, 0.7, 0);
			glPopMatrix();
		glPopMatrix();
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			giroBrazoIzq += KeyFrame[playIndex].giroBrazoIzqInc;  //paso5
			giroBrazoDer += KeyFrame[playIndex].giroBrazoDerInc;  //paso5
			i_curr_steps++;
		}
